...
class ComponentsJobDemandManagerDepositDetailTable extends Component {
  static propTypes = {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {
          this.props.flowTableVisible || this.props.refundFormVisible ?
            <SingleTable {...this.props} table={this.props.table.filter(t => (t.id === this.props.flowTableSearchData.detailId || t.id === this.props.refundFormData.detailId))} columns={columns} scroll={{ x: true }} className="tablexscroll" /> :
...
export default ComponentsJobDemandManagerDepositDetailTable;